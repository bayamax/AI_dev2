...
────────────────────────────────────────────────────────
● 投稿 Tensor を CPU 側 FP32 でキャッシュ
● Multi-processing は “spawn” 固定で CUDA-fork を回避
● Hard Neg 80 % + Easy Neg 20 %、HardNeg は 2 epoch ごと再計算
● train loss と valAUC を記録し、最良モデルを自動保存
● rank_follow_model.pt があればロードして続き学習
"""
...
def split_edges(edges):
    if TRAIN_NPY.exists() and VAL_NPY.exists():
        return (np.load(TRAIN_NPY, allow_pickle=True).tolist(),
                np.load(VAL_NPY  , allow_pickle=True).tolist())
...
def compute_acc_vecs(cache, device, ap):
    ...
    for i in range(0, len(users), 512):
        ...
        fp = torch.stack(fp).to(device)
        fm = torch.stack(fm).to(device)
...
def build_val_dl(val_edges, cache):
    ...
    return DataLoader(TripletDS(val_edges, neg, cache),
...
def train():
    ...
    for ep in range(1, NUM_EPOCHS+1):
        ...
        if ep==1 or ep%HN_INTERVAL==0:
            users, V = compute_acc_vecs(post_cache, device, model.ap)
            hard = hard_negs(users, V, train_e)
        easy = easy_negs(list(valid_users), train_e)

        dl = DataLoader(TripletDS(train_e, hard+easy, post_cache),
        ...
        for fp,fm,tp,tm,np_,nm in tqdm(dl, desc=f"E{ep}"):
            fp,fm,tp,tm,np_,nm=[x.to(device) for x in (fp,fm,tp,tm,np_,nm)]
