...
・投稿ゼロのユーザーは正例・負例とも除外
・list/tuple 混在バグ修正、tensor 生成 Warning 解消
"""
...
def compute_acc_vecs(posts, dev, ap):
    ...
    V = np.zeros((len(users), ACCOUNT_DIM), np.float32)      # ← 修正済み
    ap.eval()
    with torch.no_grad():
...
def hard_negs(users, V, pos_edges):
    ...
    sims = F.normalize(torch.tensor(V), dim=1) @ F.normalize(torch.tensor(V), dim=1).T
    neg = []
...
class TripletDS(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return (torch.tensor(fp), torch.tensor(fm),
                ...
                torch.tensor(np_), torch.tensor(nm))        # ← map → tuple
