...
# -*- coding: utf-8 -*-
"""
post_to_accountvector_ranked.py   ★Hard + Easy Neg + 完全フィルタ版
────────────────────────────────────────────
■ Hard Negative 80 % ＋ Easy Negative 20 %
■ “投稿の無いユーザー” は正例・負例とも完全除外
■ list/tuple 混在バグ解消
■ tensor 警告解消（np.stack→tensor）
"""

from pathlib import Path
import csv, random, traceback
...
HARD_TOPN     = 20         # 類似度上位候補
EASY_NEG_RATE = 0.2        # EasyNeg 割合
VALID_RATIO   = 0.15
# ────────────────────────────────────────────
...
def split_edges(edges):
    ...
    np.save(TRAIN_NPY, np.array(edges[:k], dtype=object)); np.save(VAL_NPY, np.array(edges[k:], dtype=object))
...
def compute_acc_vecs(posts, dev, ap):
    ...
    V=np.stack([posts[u][0]*0 for u in users])  # placeholder same shape
    # chunk
    with torch.no_grad():
        for i in range(0,len(users),512):
            fp,fm=zip(*(pad_posts(posts[u]) for u in users[i:i+512]))
            v=ap(torch.tensor(np.array(fp),device=dev),
                 torch.tensor(np.array(fm),device=dev)).cpu().numpy()
            V[i:i+len(v)]=v
    return users, V
...
def hard_negs(users,V,pos_edges):
    ...
    M=torch.tensor(V); M=F.normalize(M,dim=1); sims=M@M.T
    neg=[]
    for f,t in pos_edges:
        if f not in idx: continue
        i=idx[f]
        vals,ids=sims[i].topk(HARD_TOPN+1)
...
def train():
    ...
    edges=[e for e in all_edges if e[0] in valid and e[1] in valid]
    ...
    for ep in range(1,NUM_EPOCHS+1):
        ...
        easy=easy_negs(list(valid),train_e)
        ds=TripletDS(train_e, hard+easy, posts)
        dl=DataLoader(ds,batch_size=BATCH_SIZE,shuffle=True,drop_last=True)

        model.train(); tot=0
        for batch in tqdm(dl,f"E{ep}"):
            fp,fm,tp,tm,np_,nm=[x.to(dev) for x in batch]
            pos=model.score(fp,fm,tp,tm); neg=model.score(fp,fm,np_,nm)
        ...
        if avg<best:
            best,wait=avg,0
            torch.save(model.state_dict(),MODEL_PATH); log(" saved best")
        else:
            wait+=1
